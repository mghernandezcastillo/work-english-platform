...
export default function LessonView() {
  ...
  async function loadLesson() {
    ...
    try {
      ...
      // Cache check via content._v token (injected by update scripts).
      // Without it, always fetch fresh to avoid serving stale content.
      const dbMeta = parsedCache ? await supabase
        .from('lessons').select('content').eq('id', lessonId).single()
        .then(r => r.data) : null
      const cacheValid = dbMeta &&
        dbMeta.content?._v !== undefined &&
        parsedCache.content?._v !== undefined &&
        dbMeta.content._v === parsedCache.content._v
