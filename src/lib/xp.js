...
export async function awardLessonXP(userId, lessonsCompleted, streakDays) {
  try {
    ...
    // RPC may return a single value (number) or an object — handle both
    const totalXP = typeof profile === 'number' ? profile : (profile?.xp ?? 0)
    ...
    return { newXP: XP_PER_LESSON, newBadges: newBadges || [], totalXP }
  } catch (err) {
    console.error('awardLessonXP failed:', err)
    return { newXP: 0, newBadges: [] }
  }
}
